...
class Operation(object):
    ...
    def auth(self, *args):
        ...
        if auth_flag:
            self.conn.send(json.dumps(user_dic).encode("utf-8"))
    ...
    def ls(self, *args):
        ...
        self.conn.send(json.dumps(res_dic).encode("utf-8"))
    ...
    def get(self, *args):
        ...
        self.conn.send(json.dumps(res_dic).encode('utf-8'))      # 将file的判断结果发送给client
    ...
    def put(self, *args):
        ...
        while recvice_size < filesize:
            try:  # 防止 BlockingIOError: [WinError 10035] 无法立即完成一个非阻止性套接字操作。
