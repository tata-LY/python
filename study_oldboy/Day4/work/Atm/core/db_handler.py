...
def file_db_select(id, password, *args, **kwargs):    # 增加args参数，识别transfer标识，在db处理转账的时候不需要验证密码
    ...
    if os.path.exists(account_file):
        with open(account_file, "r", encoding="utf-8") as f_r:
            ...
            if account_data['password'] == password:
                return account_data
            if kwargs.get('transfer'):  # {'transfer':'transfer'}
                return account_data
            # elif len(args) > 0:
            #     if args[0] == 'transfer':
            #         return account_data
...
def db_select(id, password, *args, **kwargs):     # 增加args参数，识别transfer标识，在db处理转账的时候不需要验证密码
    database_type = settings.DATABASE
    if database_type['engine'] == 'file_storage':
        return file_db_select(id, password, *args, **kwargs)
